...
export class JsdocCommentsWalker extends Lint.RuleWalker {
  ...
  private validateJsDocComment(node: ts.Node) {
    const hasPrivateModifiers = Lint.hasModifier(
      node.modifiers,
      ts.SyntaxKind.PrivateKeyword
    );
    const hasProtectedModifiers = Lint.hasModifier(
      node.modifiers,
  ...
  // Much of this code is taken from TS 2.0. Can be removed once this
  // functionality is available.
  private getJsDocCommentsFromText(node: ts.Node, text: string) {
   ...
   const enum CharacterCodes {
    ...
    const commentRanges = (node.kind === ts.SyntaxKind.Parameter ||
      ...
      getCommentRanges(text, node.pos, false).concat(
        getCommentRanges(text, node.pos, true)
      ) :
      getCommentRanges(text, node.pos, false);
    return commentRanges ? commentRanges.filter(isJsDocComment) : [];

    function isJsDocComment(comment: ts.CommentRange) {
    ...
    function getCommentRanges(text: string, pos: number, trailing: boolean): ts.CommentRange[] {
      let result: ts.CommentRange[];
